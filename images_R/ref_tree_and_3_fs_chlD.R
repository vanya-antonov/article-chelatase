...
# Extract the REF_TREE + 3 validated fs-chlD
ref_genes <- anno_df %>% filter(!is.na(gene)) %>% pull(name)
val_genes <- anno_df %>%
  filter(org_name %in% c('Delftia acidovorans SPH-1', 'Methanocaldococcus fervens AG86', 'Methanocaldococcus sp. FS406-22')) %>%
  pull(name)
ref_tree <- keep.tip(tree, c(ref_genes, val_genes))
anno_df <- anno_df[ref_tree$tip.label,]
# ggtree(ref_tree)
...
ggsave('ref_tree_and_3_fs_chlD.pdf', path = paste0(OUT_DIR, 'ref_tree'), width = 4, height = 6)
